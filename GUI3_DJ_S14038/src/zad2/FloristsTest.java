...
public class FloristsTest {
  ...
  static int valueOf(Box box, String color) {
     double total=0;
     java.util.HashMap<String,Double> cennik=PriceList.getInstance().mapa;
for (int i = 0; i < box.zawartosc.size(); i++) {
	
	if(box.zawartosc.get(i).getColor().equalsIgnoreCase(color)){
		total=total+box.zawartosc.get(i).count*cennik.get(box.zawartosc.get(i).getNazwa());
